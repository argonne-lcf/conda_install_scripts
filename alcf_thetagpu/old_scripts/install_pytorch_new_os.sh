...
PT_INSTALL_BASE_DIR=$THISDIR/pt_master/test_new_os
...
# as of May 2021 tags cannot be built from scratch
# https://github.com/pytorch/pytorch/issues/47993
...
if [ ! -d $CUDNN_BASE ]; then
   echo $CUDNN_BASE does not exist
   exit -1
fi
...
if [ ! -d $TENSORRT_BASE ]; then
   echo $TENSORRT_BASE does not exist
   exit -1
fi
